...
from .scrollbars import CapsuleScrollBar
...
class CopyablePasswordLineEditMixin:
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        ...
        if key in (QtCore.Qt.Key.Key_Left, QtCore.Qt.Key.Key_Right) and modifiers == QtCore.Qt.KeyboardModifier.NoModifier:
            start, length = self._selected_range()
            if start >= 0 and length > 0:
                self.setCursorPosition(start if key == QtCore.Qt.Key.Key_Left else start + length)
                event.accept()
                return
...
class StyledClearLineEdit(CopyablePasswordLineEditMixin, QtWidgets.QLineEdit):
    secretRevealRequested = QtCore.pyqtSignal()
    ...
    def _polish_trailing_buttons(self):
        ...
        for button in buttons:
            ...
            if button.defaultAction() is self._eye_action and not button.property("secretEyeFilterInstalled"):
                button.installEventFilter(self)
                button.setProperty("secretEyeFilterInstalled", True)
    ...
    def _place_secret_cursor_at_end(self) -> None:
        if not self._secret_visibility_enabled:
            return
    ...
    def eventFilter(self, obj: QtCore.QObject, event: QtCore.QEvent) -> bool:
        if isinstance(obj, QtWidgets.QToolButton) and obj.defaultAction() is self._eye_action:
            if event.type() in (QtCore.QEvent.Type.MouseButtonPress, QtCore.QEvent.Type.MouseButtonRelease):
                self._place_secret_cursor_at_end()
                QtCore.QTimer.singleShot(0, self._place_secret_cursor_at_end)
    ...
    def _toggle_secret_visibility(self) -> None:
        ...
        self._suppress_view_reset = True
        ...
        try:
            ...
            if self._secret_is_visible:
                ...
                super().setEchoMode(self._secret_echo_mode)
            self._refresh_clear_action()
            self._restore_cursor_state(cursor_position, -1, 0)
        finally:
            self.setUpdatesEnabled(True)
            self.update()
        QtCore.QTimer.singleShot(0, lambda: self._restore_cursor_state(cursor_position, -1, 0))
        QtCore.QTimer.singleShot(0, lambda: setattr(self, "_suppress_view_reset", False))
    ...
    def keyPressEvent(self, event: QtGui.QKeyEvent) -> None:
        if self._secret_visibility_enabled and event.key() in (QtCore.Qt.Key.Key_Return, QtCore.Qt.Key.Key_Enter) and event.modifiers() == QtCore.Qt.KeyboardModifier.NoModifier:
            if self._secret_is_visible:
                ...
                self.secretRevealRequested.emit()
            event.accept()
            return
...
class CustomComboPopup(QtWidgets.QWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        panel_l.setContentsMargins(4, 5, 5, 7)
        ...
        self.list.setVerticalScrollBarPolicy(QtCore.Qt.ScrollBarPolicy.ScrollBarAsNeeded)
        self.list.setVerticalScrollBar(CapsuleScrollBar(QtCore.Qt.Orientation.Vertical, self.list, thickness=11, draw_outline=False, draw_track=False))
        ...
        self.list.verticalScrollBar().valueChanged.connect(self._schedule_cursor_hover_sync)
        panel_l.addWidget(self.list)

        self._apply_theme()

    def _schedule_cursor_hover_sync(self) -> None:
        if self.isVisible():
...
class CustomComboBox(QtWidgets.QComboBox):
    ...
    def showPopup(self):
        ...
        count = self._popup.list.count()
        visible_rows = min(max(count, 1), 8)
        needs_scrollbar = count > visible_rows
        ...
        scrollbar_w = 15 if needs_scrollbar else 0
        width = max(self.width(), self._popup.list.sizeHintForColumn(0) + 30 + scrollbar_w + frame_w, 170)
